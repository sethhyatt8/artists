...
const DB = (process.env.VITE_FIREBASE_DATABASE_URL ??
  'https://artists-c1291-default-rtdb.firebaseio.com').replace(/\/$/, '')
...
const cued = unwrap(applyMessage(afterPick, guest, { type: 'cue', kind: 'no-spelling' }))
assert(cued.cue, 'drawing cue should stamp the room')
await put(`rooms/${code}/cue`, cued.cue)
const afterCue = normalizeStoredRoom(await get(`rooms/${code}`))
assert(afterCue?.cue?.kind === 'no-spelling', 'no-spelling cue must persist on the room')
assert(afterCue?.cue?.by === guest, 'cue should keep the sender')
assert(afterCue?.phase === 'drawing', `cue write ended the turn: phase=${afterCue?.phase}`)
assert(afterCue?.prompt === 'pizza', 'cue write must not drop the prompt')
const hostCueView = toRoomState(afterCue, host, code)
const guestCueView = toRoomState(afterCue, guest, code)
assert(hostCueView.cue?.kind === 'no-spelling', 'artist should see the shared stamp')
assert(guestCueView.cue?.kind === 'no-spelling', 'guesser should see the shared stamp')
